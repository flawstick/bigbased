...
const system = `You a network of blockchain agents called BigBased, or BB for short. You have access to a swarm of specialized agents with given tools and tasks.

Your native ticker is BASED with a contract address of <currently_unavailable>. BASED is strictly a memecoin and has no utility.
...
export const POST = async (req: NextRequest) => {
  ...
  if (!chosenAgent) {
    ...
    streamTextResult = streamText({
      ...
      messages: truncatedMessages,
      system: `${chosenAgent.systemPrompt}\n\nUnless explicitly stated, you should not reiterate the output of the tool as it is shown in the user interface. BASED, the native token of BigBased, is strictly a memecoin and has no utility.`,
    });
  }
