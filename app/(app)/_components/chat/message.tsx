  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {message.role === "user" ? "You" : "BigBased"}
        </p>
      </div>
      <div
        className={cn(
        ...
        components={{
          ...
          ...(compressed
            ? {
                h1({ children }) {
                  return (
                    <h1 className={cn("text-lg md:text-xl font-bold")}>
                      {children}
                    </h1>
                  );
                },
                h2({ children }) {
                  return (
                    <h2 className={cn("text-md md:text-lg font-bold")}>
                      {children}
                    </h2>
                  );
                },
                h3({ children }) {
                  return (
                    <h3 className={cn("text-sm md:text-md font-bold")}>
                      {children}
                    </h3>
                  );
                },
                h4({ children }) {
                  return (
                    <h4 className={cn("text-sm md:text-sm font-bold")}>
                      {children}
                    </h4>
                  );
                },
                h5({ children }) {
                  return (
                    <h5 className={cn("text-xs md:text-xs font-bold")}>
                      {children}
                    </h5>
                  );
                },
                h6({ children }) {
                  return (
                    <h6 className={cn("text-xs font-bold")}>{children}</h6>
                  );
                },
                li({ children }) {
                ...
                p({ children, node }) {
                  const hasBlockElements = node?.children?.some(
                    (child: { type: string; tagName: string }) =>
                      child.type === "element" &&
                      ["div", "p", "blockquote", "form"].includes(
                        child.tagName,
                      ),
                  );
