...
const WithdrawLiquidityCall: React.FC<Props> = ({ toolCallId, args }) => {
  ...
  const { data: pool, isLoading: isPoolLoading } = useRaydiumPoolByMint(
    args.mint,
  );
  ...
  useEffect(() => {
    if (pool && pool.type !== "Standard") {
      addToolResult(toolCallId, {
        message: `BigBased does not support ${pool?.type} pools yet.`,
      });
    }
  }, [pool]);
    ...
    <Card className="w-full p-2">
      {isPoolLoading && !pool ? (
        ...
        pool.type === "Standard" ? (
          ...
          <div>
            <p>BigBased does not support {pool.type} pools yet.</p>
          </div>
        )
      ) : (
        <div>
